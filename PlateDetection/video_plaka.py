...
def _draw_overlay(frame, tracks, recent_plates, video_sec, realtime_ratio, proc_fps=None):
    ...
    # Üst-sağ köşe: kaynak videonun çözünürlüğü + gerçek işlem hızı (FPS)
    corner = f"{W}x{H}"
    if proc_fps is not None:
        corner += f"  |  {proc_fps:.1f} FPS (islem)"
    (tw, th), _ = cv2.getTextSize(corner, cv2.FONT_HERSHEY_SIMPLEX, 0.55, 1)
    cv2.rectangle(view, (view.shape[1] - tw - 16, 0), (view.shape[1], th + 14), (0, 0, 0), -1)
    cv2.putText(view, corner, (view.shape[1] - tw - 8, th + 6),
